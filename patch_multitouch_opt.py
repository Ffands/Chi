...
# Patch performGestureForNodes
func_old = """    private fun performGestureForNodes(activeNodes: List<TargetNode>) {
...
func_new = """    private fun performGestureForNodes(activeNodes: List<TargetNode>) {
        if (enableMultitouch) {
            val builder = GestureDescription.Builder()
            for (node in activeNodes) {
                val path = Path()
...
# Add load to UIManager settings
